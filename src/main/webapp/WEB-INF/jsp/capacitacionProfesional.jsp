	...
	<div class="d-sm-flex">
		<div class="card col-sm-4">
			...
			<table class="table table-striped">
				...
				<c:forEach items='${listadocapacitacion}' var='capacitacion'>
					<tr>
						<td>${capacitacion.getFechayhora()}</td>
						<td>${capacitacion.getTema()}</td>
						<td>${capacitacion.getContenido()}</td>
						<td>${capacitacion.getProfesional().getNombre()} ${capacitacion.getProfesional().getApellido()} </td>
						<td>${capacitacion.getCliente().getNombreEmpresa()}</td>
											<td>
			<a class="btn btn-danger" href="eliminarcapacitacion/${capacitacion.getId_capacitacion()}">Eliminar</a> &nbsp;
			<a class="btn btn-info" href="editarcapacitacion/${capacitacion.getId_capacitacion()}">Editar</a> &nbsp;
		</td>
					</tr>
				</c:forEach>
				</tbody>
